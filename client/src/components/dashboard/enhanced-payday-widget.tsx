...
export default function EnhancedPaydayWidget() {
  ...
  const updatePreferences = useMutation({
    ...
    onSuccess: () => {
      ...
      toast({
        ...
        description: "Your payday preferences have been saved.",
      });
    },
    onError: () => {
      toast({
  ...
  const calculateDaysUntilPayday = () => {
    ...
    // If payday has passed this month, calculate for next occurrence
    if (payday < today) {
      ...
      <CardContent className="space-y-3">
        {daysUntilPayday !== null ? (
          <>
            <div className="text-center">
              ...
              <div className="text-xs text-muted-foreground">
                {daysUntilPayday === 0 ? 'Today!' : daysUntilPayday === 1 ? 'day' : 'days'}
              </div>
            </div>
            ...
            <div className="text-center">
              <Badge 
                variant={daysUntilPayday <= 3 ? "default" : "secondary"}
                className="text-xs"
              >
                {daysUntilPayday <= 3 ? "Soon!" : "Scheduled"}
              </Badge>
            </div>
          </>
        ) : (
          <div className="text-center text-xs text-muted-foreground py-4">
            ...
            <p>Set your payday to track upcoming payments</p>
          </div>
        )}
      </CardContent>
    </Card>
